...
class IndexController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if(@$_GET['code']){
            $member = Db::table('members')->where('invite_code',$_GET['code'])->first();
            if($member){
                $request->session()->put('daili_id',$member->id);
            }
    ...
    public function notice(Request $request)
    {
        ...
        $data = $mod->where('on_line', 0)->orderBy('sort', 'asc')->orderBy('created_at', 'desc')->get();
        return view('web.notice', compact('data'));
    }
    
    public function sport()
    {
        return view('web.sport');
    }
  
   public function live()
    {
        return view('web.live');
    }
    
       public function eGame()
    {
        return view('web.eGame');
    }
    
    public function eBattle()
    {
        return view('web.ebattle');
    }
    
    public function keno()
    {
        return view('web.keno');
    }
    
    public function fishing()
    {
 
        return view('web.fishing');
    }
    ...
    public function register_one(Request $request)
    {
        ...
        return view('web.register_one', compact('i_code', 'register_name','setting'));
    }
    public function login2(Request $request)
    {
        ...
        $register_name = $request->has('register_name')?$request->get('register_name'):'';
        $setting = Db::table('system_config')->first();
        return view('web.login_one', compact('i_code', 'register_name','setting'));
    }
...
//            ]);
	...
	public function checkqipai(){
		$list=Api::whereIn('api_name',array('KY','WGQP','TH','761','MT','JS','SW','NW','NEWVG','AS','HG','IMQP','LEG','DSQP','FG','YG','GG'))->get();
		$res=[];
		foreach($list as $v){
			
			$res[$v->api_name]=$v->on_line?0:1;
		}
