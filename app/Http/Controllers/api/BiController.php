...
class BiController extends WebBaseController
{
    ...
    public function balance($platformCode, $username)
    {
        ...
        if (!$member_api)
        {
            $platformCode = $platformCode == 'BBIN' ? 'BBIN': $platformCode;
		...
		if($sys->mz_open){
            $res = json_decode($this->service->mz_balances($username), TRUE);
        }else{
    ...
    public function withdrawal($platformCode,$username, $amount, $amount_type = 'money')
    {
        ...
        if (is_array($res) && $res['retCode'] == 0)
        {
            ...
            $return['Code'] = $res['retCode'];
            $return['Message'] = '错误代码 '.$res['StatusCdoe'].',错误信息 '.$res['Message'].' 请联系客服';
        }
    ...
    public function credit($api_name)
    {
        ...
        $res = json_decode($this->service->BusinessBalance($api_name), TRUE);

        //print_r($res);exit;
        //暂时只接YC接口
        if ($res['retCode'] == 0)
        {
            //$api_name = $api_name == 'BBIN'?'BBIN':$api_name;
            $money = $res['Balance'];
            $api->update([
                'api_money' => $money
            ]);
