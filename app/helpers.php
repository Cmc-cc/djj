...
',$env);
        ...
        foreach ($env as $k=>$v){
            $xcode = explode('=',$v);
            if($xcode[0]=='ADMIN_URL'){
                $admin_url = $xcode[1];
            }
        }
        
        
    if($_SERVER['HTTP_HOST']==$admin_url){
        $msg = is_array($msg)?json_encode($msg):json_encode(['成功']);
    }else{
...
',$env);
    ...
    if($_SERVER['HTTP_HOST']==$admin_url){
        $msg = is_array($msg)?json_encode($msg):json_encode([$msg]);
    }else{
        $msg = is_array($msg)?json_encode($msg):$msg;
    }
...
function getset($keyword){
    $set = Db::table('system_config')->first();
    return @$set->$keyword?$set->$keyword:'';
}