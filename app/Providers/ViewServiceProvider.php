...
{
    protected $auth_user,$admin_aside,$admin,$auth_member,$web,$web_header,$auth_daili,$daili_aside,$wap,$system_notice,$web_footer,$mb_path;
    ...
    public function boot()
    {
        ...
        view()->composer('*', function ($view) {
			...
			$_api_list_1 = Api::where('on_line', 0)->orderBy('created_at', 'desc')->pluck('api_title', 'id')->toArray();
			$_api_list_all = Api::orderBy('created_at', 'desc')->pluck('api_title', 'id')->toArray();
            $view->with(compact('_system_config', '_api_list','_api_list_1','_api_list_all'));
        });
        ...
        view()->composer($this->web, function ($view) {
			...
			if($_member){
				$center_money=$_member->money;
				$sys = SystemConfig::findOrfail(1);
				
				if($sys->mz_open){
					$WgService=new WgService();
					$ret=$WgService->mz_balances($_member->name);
					$mz_money=json_decode($ret,1);
					if(!$mz_money['retCode']){
						$_member->all_money=$mz_money['retMsg']+$center_money;
					}
				}else{
					
					$_member->all_money=$_member->money;
				}
			}
...
//            $_member->all_money= sprintf('%.2f',$_member->money + $center_money);
        ...
        view()->composer($this->mb_path,function ($view){
