var biCommon = {
        ...
        biCommon.ajax('post', "", "/api/MemberApi?ApiInterface=bindbank", function (res) {
            if (res.retCode == 0) {
                ...
                if (retMeg.mybankname != "") {
                    ...
                    $(".personal-details__preferences input").hide();
                    $(".personal-details__preferences a").show();
                }
            } else {
                notify(res.retMsg);
            }
    ...
    getuserplatform: function (obj) {//一键查询所有平台余额
        biCommon.ajax('post', "", "/api/CashOperationApi?ApiInterface=getuserplatform", function (res) {
            if (res.retCode == 0) {
                 findPlateform(JSON.parse(res.retMsg));
            } else {
                notify(res.retMsg);
            }
            disabledT("2", obj);
    ...
    getGameBalance: function (platformCode, obj) {//获取单个平台的游戏余额
        ...
        biCommon.ajax('post', data, "/api/CashOperationApi?ApiInterface=ref_ed", function (res) {
            if (res.retCode == 0) $(obj).parent().parent().find("b").html(res.retMsg);
            else notify(res.retMsg);
            $(obj).removeClass("add");
        })
	},
            ...
            if (res.retCode == 0) {
                if (type == "fishindex") {
                    ...
                    for (var i = 0; i < res.retData.length; i++) {
                        ...
                        //fishQuick += '<div data - v - 2d10ac7a = "" platformCode="' + platform[i].api_name + '" gameType="' + platform[i].gameType + '" onclick="return landing(this);" relative = "" class="game-box" > <div data-v-2d10ac7a="" class="border-top"></div> <div data-v-2d10ac7a="" class="game-box-content"><div data-v-2d10ac7a="" ><img data-v-2d10ac7a="" src="/img/fish/' + platform[i].api_Name.toLowerCase() + '_' + platform[i].onlyImg + '.png" alt=""></div><p data-v-2d10ac7a="">' + platform[i].chineseName + '</p></div></div>';
                        fishQuick += '<div class="col-3 col-sm-6"><a href = "javascript:void(0)" platformCode="' + platform[i].api_Name + '" gameType="' + platform[i].gameType + '" onclick="return landing(this);" relative = "" class="game-box"><img src="/Areas/Wap9/Content/images/BIimg/fish/' + platform[i].api_Name.toLowerCase() + '_' + platform[i].onlyImg + '.png" /><div class="text"><h4>' + platform[i].chineseName + '</h4></div></a ></div >';
