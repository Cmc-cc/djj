var biCommon = {
    ...
    getGamePlates: function (num, type) {
        ...
        type = type == undefined ? "AG" : type;
        var apiurl = "";
        if (num == 1 || num == 4) apiurl = "getapiassort";
        else apiurl = "getapi";
            ...
            if (res.retCode == 0) {//1首页，2转账，3下注记录
                switch (num) {
                    ...
                    case 3:
                        ...
                        break;
                    case 4:
                        layoutPlatform(res.retData,type);
                        break;
                }
            } else {
                notify(res.retData);
        ...
        biCommon.ajax('post', "", "/api/MemberApi?ApiInterface=bindbank", function (res) {
            if (res.retCode == 0) {
                ...
                if (retMeg.mybankname != "") {
                    ...
                    else if (type == 3) {//1个人资料、2绑定银行卡、3提款
                        $(openBank).val(retMeg.mybankname);
                        $(bankCard).val(retMeg.mybanknum);
                        $(bankMeg).val(retMeg.mybankaddress);
                        $(TrueName).val(retMeg.myname);
                    } else {
                        $("#newbtn").hide();
                        $(TrueName).text(retMeg.myname);
                    ...
                    if (type != 1) $(".wtdCard-content").hide();
                    if (type == 3 || type == 1) {
                        window.location.href = "/Index/wtdCard";
                    }
                    $("#UserName").val(retMeg.myname);
                    $(".person-bank input").hide();
                    $(".person-bank a").show();
                }
            } else {
                notify(res.retMsg);
    ...
    getuserplatform: function (obj,type) {//一键查询所有平台余额
        biCommon.ajax('post', "", "/api/CashOperationApi?ApiInterface=getuserplatform", function (res) {
            if (res.retCode == 0) {
                if (type != undefined) apiBalanceIndex(JSON.parse(res.retMsg));
                else findPlateform(JSON.parse(res.retMsg), type);
            } else {
                notify(res.retMsg);
            }
            disabledT("2", obj);
    ...
    getGameBalance: function (platformCode, obj) {//获取单个平台的游戏余额
        ...
        biCommon.ajax('post', data, "/api/CashOperationApi?ApiInterface=ref_ed", function (res) {
            if (res.retCode == 0) $(obj).parent().parent().find("span.button-value.yue").html(res.retMsg);
            else notify(res.retMsg);
            $(obj).removeClass("add");
        })
	},
    rollout: function (obj, type) {//一键转出(转出所有平台余额)
        biCommon.ajax('post', "", "/api/CashOperationApi?ApiInterface=onetouchtransfer", function (res) {
            if (res.retCode == 0) {
                $("#yijianrefsh").click();
                notify("转出所有平台余额成功！");
                if (type == 1) biCommon.getuserplatform("", 3);
            } else {
                notify(res.retMsg);
            }
            disabledT(2, obj);
    ...
    bankinfo: function (type) {//获取银行卡、二维码收款
        ...
        biCommon.ajax('post', data, "/api/RecordingApi?ApiInterface=bankinfo", function (res) {
            if (res.retCode == 0) {
                if ($("#deposithtml").length > 0) {
                    if (type == "bank" && biCommon.isEmptyObject(res.retData) == false) {
                        $("#gschongzhi li").eq(0).show();
                    } else if (res.retData != null) {
                        for (var i = 0; i < res.retData.length; i++) {
                            if (res.retData[i].type == type) $("#mgmChongZhi li." + type).show();
                        }
                    }
                } else {
                    depositFun(type, res.retData);
    ...
    bankSave: function (money,name) {//银行卡存款
        ...
        biCommon.ajax('post', data, "/api/CashOperationApi?ApiInterface=moneyinhk", function (res) {
            if (res.retCode == 0) {
                ...
                $("#commoney0").val("");
                $("#Accounts").val("");
                $("#complaymoney").val(money);
                $("#complayname").val(name);
                nextstep(2);
            } else {
                notify(res.retMsg);
            }
    ...
    qrcodeSave: function (par, money, name) {//二维码存款
        ...
        biCommon.ajax('post', data, "/api/CashOperationApi?ApiInterface=moneyinqr", function (res) {
            if (res.retCode == 0) {
                $("#Payqrcode").attr("src", res.retMsg);
                $(".content").hide();
                $(".nytop").hide();
                $(".body").show();
                $(".dep-warnings").show();
                $(".dep-warnings").find(".zhuanCode").html(res.retMsg.split('?')[1]);
            } else {
                notify(res.retMsg);
            }
        })
        ...
        biCommon.ajax('post', data, "/api/RecordingApi?ApiInterface=payinfolist", function (res) {
            if (res.retCode == 0) {
                if (biCommon.isEmptyObject(res.retData) == false) {
                    ...
                    //$("#depositTitle .clearboth div").eq(5).css({ "display": "inline-block" });
                }
            } else {
                notify(res.retMsg);
            }
    ...
    getApiFish: function (type) {//获取捕魚列表
        ...
        biCommon.ajax('post', data, "/api/NotLoggedInApi?ApiInterface=getapifish", function (res) {
            if (res.retCode == 0) {
                if (type == "quickFish") {
                    var fishQuick = "";
                    for (var i = 0; i < res.retData.length; i++) {
                        var platform = res.retData;
                        fishQuick += '<div data-v-2d10ac7a = "" platformCode="' + platform[i].api_Name + '" gameType="' + platform[i].gameType + '"  gameId="' + platform[i].gameId + '" gameName="' + platform[i].gameName + '" onclick="return landing(this);" relative = "" class="game-box" > <div data-v-2d10ac7a="" class="border-top"></div> <div data-v-2d10ac7a="" class="game-box-content"><div data-v-2d10ac7a="" ><img data-v-2d10ac7a="" src="/img/fish/' + platform[i].api_Name.toLowerCase() + '_' + platform[i].onlyImg + '.png" alt=""></div><p data-v-2d10ac7a="">' + platform[i].chineseName + '</p></div></div>';
                        //$("#GamesList").html(fishQuick);                        
                        $("#Indexfish").html(fishQuick);
                    }
    ...
    readWebsitemail: function (id, obj) {//阅读站内信
        ...
        biCommon.ajax('post', data, "/api/RecordingApi?ApiInterface=readwebsitemail", function (res) {
            if (res.retCode == 0) {
                if (obj != "undefined") {
                    $(obj).find("td img").attr('src', "/Areas/Wap7/Content/images/main/message1.png");
                }
                if (id == "0") {
                    $("#siteMail tr").each(function (x, y) {
                        $(y).find("td img").attr('src', "/Areas/Wap7/Content/images/main/message1.png");
                    });
                }
                $("#siteMail tr input").prop("checked", false);
                id = id == "0" ? $("#mailbox b").text(0) : $("#mailbox b").text(parseInt($("#mailbox b").text()) - 1);
            } else {
                notify(res.retMsg);
            }
        })
