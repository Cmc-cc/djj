var biCommon = {
    ...
    getGamePlates: function (num) {
        ...
        biCommon.ajax('post', "", "/api/NotLoggedInApi?ApiInterface=" + apiurl, function (res) {
            if (res.retCode == 0) {//1首页，2转账，3下注记录3、
                switch (num) {
                    case 1:
            ...
            if (res.retCode == 0) {
                ...
                if (retMeg.mybankname != "") {
                    if (type == 1 || type == 3) {//1个人资料、2绑定银行卡、3提款
                        ...
                        $("#bankcardForm input").attr("readOnly", "true");
                        $("#bankcardForm small").hide();
                    }
                } else {
	...
	addCards:function(cardName,cardNum,cardAddress){//绑定银行卡
        ...
        biCommon.ajax('post', data, "/api/MemberApi?ApiInterface=bindbankadd", function (res) {
            if (res.retCode == 0) {
                notify("绑定银行卡成功");
                window.location.href = "/Index/withdraw";
            } else {
                notify(res.retMsg);
            }
        })
        ...
        biCommon.ajax('post', data, "/api/RecordingApi?ApiInterface=payinfolist", function (res) {
            if (res.retCode == 0) {
                if (biCommon.isEmptyObject(res.retData) == false) {
                    ...
                    if (res.retData.length>0) $("#mgmChongZhi .list_pay_item_w").eq(5).css({ "display": "inline-block" });
                }
            } else {
                notify(res.retMsg);
            }
            ...
            if (res.retCode == 0) {
                if (type == "quickFish") {
                    ...
                    for (var i = 0; i < res.retData.length; i++) {
                        ...
                        fishQuick += '<li class="mui-table-view-cell mui-media mui-col-xs-4 mui-col-sm-3"><a href="javascript:;" onclick="return landing(this);" platformCode="' + platform[i].api_Name + '" gameType="' + platform[i].gameType + '"><span class="api-item api-icon-3-21 site953"> <img src="/img/fish/' + platform[i].api_Name.toLowerCase() + '_' + platform[i].onlyImg + '.png" /></span><div class="mui-media-body">' + platform[i].api_Name + platform[i].chineseName + '</div> </a></li>';
                    }
