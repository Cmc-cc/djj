...
function platformIndexs(platform) {
    ...
    var sport = "", lottery = "", live = "", slot = "", chess = "", gameTitle = "",competition="",lotteryCount=0;
    for (var i = 0; i < platform.length; i++) {
        var name = getTypeCh(platform[i].plattype);
        if (platform[i].plattype == "sport") {
            sport += '<li class="mui-table-view-cell mui-media mui-col-xs-4 mui-col-sm-3"><a href="javascript:;" onclick="return landing(this);" platformCode="' + platform[i].api_name + '" gameType="' + platform[i].gameType + '"><span class="api-item api-icon-3-21 site953"> <img src="/Areas/Wap5/Content/images/BIimg/' + platform[i].api_name.toUpperCase() + '.png" /></span><div class="mui-media-body">' + platform[i].api_title + name +'</div> </a></li>';
        } else if (platform[i].plattype == "lottery") {
            lotteryCount++;
            lottery += '<li class="swiper-slide apiType_lottery" plateform="' + platform[i].api_name + '" gameType="' + platform[i].gameType + '"><b class="mui-tab-item sub_list_nav"><i class="api-icon-4-22-1"></i><span>' + platform[i].api_title + name + '</span></b></li>';
        } else if (platform[i].plattype == "live") {
            live += '<li class="mui-table-view-cell mui-media mui-col-xs-4 mui-col-sm-3"><a href="javascript:;" onclick="return landing(this);" platformCode="' + platform[i].api_name + '" gameType="' + platform[i].gameType + '"><span class="api-item api-icon-3-21 site953"> <img src="/Areas/Wap5/Content/images/BIimg/' + platform[i].api_name.toUpperCase() + '.png" /></span><div class="mui-media-body">' + platform[i].api_title + name + '</div> </a></li>';
        } else if (platform[i].plattype == "chess") {
            chess += '<li class="mui-table-view-cell mui-media mui-col-xs-4 mui-col-sm-3"><a href="javascript:;" onclick="return landing(this);" platformCode="' + platform[i].api_name + '" gameType="' + platform[i].gameType + '"><span class="api-item api-icon-3-21 site953"> <img src="/Areas/Wap5/Content/images/BIimg/' + platform[i].api_name.toUpperCase() + '.png" /></span><div class="mui-media-body">' + platform[i].api_title + name + '</div> </a></li>';
        } else if (platform[i].plattype == "competition") {
            competition += '<li class="mui-table-view-cell mui-media mui-col-xs-4 mui-col-sm-3"><a href="javascript:;" onclick="return landing(this);" platformCode="' + platform[i].api_name + '" gameType="' + platform[i].gameType + '"><span class="api-item api-icon-3-21 site953"> <img src="/Areas/Wap5/Content/images/BIimg/' + platform[i].api_name.toUpperCase() + '.png" /></span><div class="mui-media-body">' + platform[i].api_title + name + '</div> </a></li>';
            ...
            slot += '<li class="mui-table-view-cell mui-media mui-col-xs-4 mui-col-sm-3"><a href="javascript:;" onclick="gameOpen(\'' + platform[i].api_name + '\')"><span class="api-item api-icon-3-21 site953"> <img src="/Areas/Wap5/Content/images/BIimg/' + platform[i].api_name.toUpperCase() + '.png" /></span><div class="mui-media-body">' + platform[i].api_title + name + '</div> </a></li>';
        } else if (platform[i].plattype == "liuhecai") {
            //lottery += '<li><a href="javascript:;" onclick="return landing(this);" platformCode="' + platform[i].api_name + '" gameType ="' + platform[i].gameType + '"><img src="/Areas/Wap5/Content/images/BIimg/' + platform[i].api_name.toUpperCase() + '6.png" alt="' + platform[i].api_name + '六合彩"><span class="item-name">' + platform[i].api_title + '六合彩</span></a></li>';
        } else if (platform[i].plattype == "guanfangcai") {
           // lottery += '<li><a href="javascript:;" onclick="return landing(this);" platformCode="' + platform[i].api_name + '" gameType ="' + platform[i].gameType + '"><img src="/Areas/Wap5/Content/images/BIimg/' + platform[i].api_name.toUpperCase() + 'gfc.png" alt="' + platform[i].api_name + '官方彩"><span class="item-name">' + platform[i].api_title + '官方彩</span></a></li>';
        }
    }
    $("#indexSport").html(sport);
    $("#indexLottery").html(lottery);
    
    indexLotterySwiper(lotteryCount);
    $("#indexLive").html(live);
    $("#indexChess").html(chess);
    $("#indexSlot").html(slot); 
    $("#indexCompetition").html(competition);
	showQuickData($("#indexLottery li").attr("plateform"), 0);
}
...
function transactionChange(obj, num) {
    ...
    $(".mine-table table").each(function (x, y) {
        if ($(y).attr("dataSource") == $("#historySel option:selected").attr("dataSource")) {
            $(y).show();
            return false;
        }
    })
    biCommon.mconvertrecord($("#historySel option:selected").attr("type"), $("#startTime").val(), $("#endTime").val(), 1);
}
function getMeg(obj) {
    ...
    var type = $("#historySel option:selected").attr("type");
    var page = "";
    if (obj == "-1") {
...
function getPlatform(platform){//获取游戏列表
	...
	var arr = "";
    for (var i = 0; i < platform.length; i++) {
        str += "<div class='each_item'><p class='item_name' platform='" + platform[i].api_name + "'> <span>" + platform[i].api_mainname + "</span><b class='item_num'>10</b><i onclick='chaxun(\"" + platform[i].api_name + "\",this)'></i></p><p class='item_name'><button onclick='yijianinout(\"" + platform[i].api_name + "\",this,\"in\")'>一键转入</button><button onclick='yijianinout(\"" + platform[i].api_name + "\",this,\"out\")' bgcolor='#0ec392'>一键转出</button> </p></div>";     
    }
    $("#limitplace").html(str);
    $("#yijianrefsh").click();
}
...
function changgepage(obj, num) {
    ...
    $("#mgmChongZhi .list_pay_item").removeClass("cur");
...
function depositFun(type, retData) {
    ...
    switch (type) {
        case "bank":
            if (biCommon.isEmptyObject(retData) == false) {
                $("#mgmChongZhi .list_pay_item_w").eq(0).css({ "display": "inline-block" });
                for (var i = 0; i < retData.length; i++) {
        ...
        case "wx":
            if (retData != null) {
                ...
                if (str != "") {
                    $("#mgmChongZhi .list_pay_item_w").eq(1).css({ "display": "inline-block" });
                    $("#wxSel").html(str);
                }
            } else {
                notify("暂无线下微信汇款！");
        ...
        case "zfb":
            if (retData != null) {
                ...
                if (str != "") {
                    $("#mgmChongZhi .list_pay_item_w").eq(2).css({ "display": "inline-block" });
                    $("#zfbSel").html(str);
                }
            } else {
                notify("暂无线下支付宝汇款！");
        ...
        case "qqpay":
            if (retData != null) {
                ...
                if (str != "") {
                    $("#mgmChongZhi .list_pay_item_w").eq(3).css({ "display": "inline-block" });
                    $("#qqSel").html(str);
                }
            } else {
                notify("暂无线下QQ汇款！");
        ...
        case "ysf":
            if (retData != null) {
                ...
                if (str != "") {
                    $("#mgmChongZhi .list_pay_item_w").eq(4).css({ "display": "inline-block" });
                    $("#yunsfSel").html(str);
                }
            } else {
                notify("暂无云闪付汇款！");
    ...
    if (countDeposit == 5) {
        var count = 0;
        $("#mgmChongZhi .list_pay_item_w").each(function (x, y) {
            if (!$(y).is(":hidden")) {
                if (count == 0) {
                    count++;
                    $(y).click();
...
function chooseBlance(money,id) {
    $(id).val(money);
}
function changebank() {
...
function compaly() {
    if ($("#commoney").val() != "" && parseFloat($("#commoney").val().trim()) >= $("#userNameController").attr("minmoney") && parseFloat($("#commoney").val().trim()) <= $("#userNameController").attr("maxmoney")) {
        if ($("#mgmChongZhi .list_pay_item.cur").parent().index() != 0) {
            var name = "";
            $(".bank_list_i_w").each(function(x,y){
                if (!$(y).is(":hidden")) name = $(y).find("select option:selected").text();
            })
            biCommon.qrcodeSave($("#mgmChongZhi .list_pay_item.cur").parent().attr("partype"), $("#commoney").val(), name );
        } else {
            if ($("#Accounts").val().trim() != "") {
                biCommon.bankSave($("#commoney").val(), $("#Accounts").val());
            } else {
                notify("请输入汇款人姓名！");
            }
        }
...
function getFishGame(retDate,type) {
    ...
    for (var i = 0; i < retDate.length; i++) {
        if (type == "fish") str += '<div class="mui-col-xs-4"><a href="javascript:void(0)" onclick="return landing(this);" platformCode="' + retDate[i].api_Name + '" gameType="' + retDate[i].gameType + '" gameId="' + retDate[i].gameId + '" gameName="' + retDate[i].gameName + '"><div class="img-wrap"><img src="/img/fish/' + retDate[i].api_Name.toLowerCase() + '_' + retDate[i].onlyImg + '.png" /></div><p>' + retDate[i].api_Name + retDate[i].chineseName + '</p></a></div>';       
        else str += '<div class="mui-col-xs-4"><a href="javascript:void(0)" onclick="return landing(this);" platformCode="' + retDate[i].api_Name + '" gameType="' + retDate[i].gameType + '" gameId="' + retDate[i].gameId + '" gameName="' + retDate[i].gameName + '"><div class="img-wrap"><img src="/img/imgPC/' + retDate[i].api_Name.toLowerCase() + "/" + retDate[i].onlyImg + '.png" /></div><p>' + retDate[i].chineseName + '</p></a></div>';       
...
function landing(obj) {
    if ($("#userNameController").val() != "") {
        ...
        if ($(obj).attr("platformCode") == "IG" || $(obj).attr("platformCode") == "IGA") {
            var IGGameType = $("#indexLottery li[plateform='"+platformCode+"']").attr("gametype");
...
function landingslocation(obj) {//页面跳转
    if ($("#userNameController").val() != "") {
        goToUrl($(obj).attr("href"), "", $(obj).attr("hre"));
	} else {
		notify("请先登录游戏！");
		return false;
	}
...
function addHb() {
    ...
    num++;
    $(".couten").append("<li class='li" + num + "' ><a href='javascript:;'><img src='/Areas/Wap5/Content/images/main/hb_" + hb + ".png'></a></li>");
    $(".li" + num).css({
        "left": Left,
    });
    $(".li" + num + " a img").css({
