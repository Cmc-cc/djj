var biCommon = {
    ...
    getUserMeg: function (UserName, TrueName, Phone, openBank, bankCard, bankMeg) {//获取用户资料
        ...
        biCommon.ajax('post', "", "/api/MemberApi?ApiInterface=memberInfo", function (res) {
            if (res.retCode == 0) {
                ...
                $(UserName).text(retMeg.Username);
                $(TrueName).text(retMeg.Myname);
                $(Phone).text(retMeg.Myphone);
            } else {
                notify(res.retMsg);
            }
        })
    ...
    getGameBalance: function (platformCode, obj) {//获取单个平台的游戏余额
        ...
        biCommon.ajax('post', data, "/api/CashOperationApi?ApiInterface=ref_ed", function (res) {
            if (res.retCode == 0) $(obj).parent().find("b").html(res.retMsg);
            else notify(res.retMsg);
            $(obj).removeClass("add");
        })
    },
        ...
        biCommon.ajax('post', data, "/api/RecordingApi?ApiInterface=payinfolist", function (res) {
            if (res.retCode == 0) {
                if (biCommon.isEmptyObject(res.retData) == false) {
                    ...
                    if (biCommon.isEmptyObject(name) == false) {
            ...
            if (res.retCode == 0) {
                if (type == "quickFish") {
                    ...
                    for (var i = 0; i < res.retData.length; i++) {
                        ...
                        fishQuick += ' <div class="col-xs-6 menu-btn-container" ><a href="javascript:;" platformCode="' + platform[i].api_Name + '" gameType="' + platform[i].gameType + '" onclick="return landing(this);" gamename="' + platform[i].gameName + '"><div class="menu-btn" style="text-align:center;"><div class="common-container"><div class="col-xs-6"><img src="/img/fish/' + platform[i].api_Name.toLowerCase() + '_' + platform[i].onlyImg + '.png" alt="' + platform[i].api_Name + platform[i].chineseName + '"></div><div class="col-xs-6"> <span>' + platform[i].api_Name + platform[i].chineseName + '</span></div></div></div></a></div>';
                    }
