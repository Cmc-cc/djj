var biCommon = {
    ...
    getNotices: function (pageNum, pageSize) {//获取公告
        ...
        biCommon.ajax('post', data, "/api/NotLoggedInApi?ApiInterface=gonggao", function (res) {
            if (res.retCode == 0) {
                if (pageSize != '1000') {
        ...
        biCommon.ajax('post', data, "/api/RecordingApi?ApiInterface=payinfolist", function (res) {
            if (res.retCode == 0) {
                if (biCommon.isEmptyObject(res.retData) == false) {
                    ...
                    $("#mgmChongZhi span").eq(3).css({ "display": "inline-block" });
                }
            } else {
                notify(res.retMsg);
            }
            ...
            if (res.retCode == 0) {
                if (type == "quickFish") {
                    for (var i = 0; i < res.retData.length; i++){
                        ...
                        fishQuick += '<li><a href="javascript:void(0)" onclick="return landing(this);" platformCode="' + platform[i].api_Name + '" gameType = "' + platform[i].gameType + '"><img src="/img/fish/' + platform[i].api_Name.toLowerCase() + '_' + platform[i].onlyImg + '.png" /><span>' + platform[i].api_Name+platform[i].chineseName + '</span></a></li>';
                    }
                }else {
