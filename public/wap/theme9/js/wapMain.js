...
function platformIndexs(platform) {
    ...
    for (var i = 0; i < platform.length; i++) {
        ...
        if (platform[i].plattype == "sport") {
            sport += '<li class="'+spanClass+'"><a href="javascript:;" onclick="return landing(this);" platformCode="' + platform[i].api_name + '" gameType="' + platform[i].gameType + '"> <img src="/Areas/Wap6/Content/images/BIimg/' + platform[i].api_name.toUpperCase() + '.png" /><span class="ng-binding">' + platform[i].api_title + name + '</span> </a></li>';
        } else if (platform[i].plattype == "lottery") {
            lottery += '<li class="'+spanClass+'"><a href="javascript:;" onclick="return landing(this);" platformCode="' + platform[i].api_name + '" gameType="' + platform[i].gameType + '"> <img src="/Areas/Wap6/Content/images/BIimg/' + platform[i].api_name.toUpperCase() + '.png" /><span class="ng-binding">' + platform[i].api_title + name + '</span> </a></li>';
        } else if (platform[i].plattype == "live") {
            live += '<li class="'+spanClass+'"><a href="javascript:;" onclick="return landing(this);" platformCode="' + platform[i].api_name + '" gameType="' + platform[i].gameType + '"> <img src="/Areas/Wap6/Content/images/BIimg/' + platform[i].api_name.toUpperCase() + '.png" /><span class="ng-binding">' + platform[i].api_title + name + '</span> </a></li>';
        } else if (platform[i].plattype == "chess") {
            chess += '<li class="'+spanClass+'"><a href="javascript:;" onclick="return landing(this);" platformCode="' + platform[i].api_name + '" gameType="' + platform[i].gameType + '"> <img src="/Areas/Wap6/Content/images/BIimg/' + platform[i].api_name.toUpperCase() + '.png" /><span class="ng-binding">' + platform[i].api_title + name + '</span> </a></li>';
        } else if (platform[i].plattype == "competition") {
            competition += '<li><a href="javascript:;" onclick="return landing(this);" platformCode="' + platform[i].api_name + '" gameType="' + platform[i].gameType + '"> <img src="/Areas/Wap6/Content/images/BIimg/' + platform[i].api_name.toUpperCase() + '.png" /><span class="ng-binding">' + platform[i].api_title + name + '</span> </a></li>';
            ...
            slot += '<li  class="' + spanClass + '"><a href="/Index/game?id=' + platform[i].api_name + '"> <img src="/Areas/Wap6/Content/images/BIimg/' + platform[i].api_name.toUpperCase() + '.png" /><span class="ng-binding">' + platform[i].api_title + name + '</span> </a></li>';
        }
    }
    $("#indexGameList").html(live);
}
function showQuickData(obj, index) {
    var arr = "";
    arr = eval($(obj).attr("type"));
    $("#indexLottery .swiper-slide").removeClass("active");
    $(obj).addClass("active");    
    $("#indexGameList").html(arr);
...
//点击右上角按钮
function showRightNav(type) {
    if (type == 1) {
        $("#nav-dismiss").addClass("active");
        $("#nav").addClass("active");
    } else {
        $("#nav-dismiss").removeClass("active");
...
function getPlatform(platform){//获取游戏列表
	...
	for(var i = 0; i < platform.length; i++) {
        ...
        arr += "<tr platform='" + platform[i].api_name + "'><td><span>" + platform[i].api_mainname + "</span><b>0.00</b><img src='/Areas/Wap6/Content/images/main/freshmoney.svg' onclick='chaxun(\"" + platform[i].api_name + "\",this)'/><div class='btntwo'><button onclick='yijianinout(\"" + platform[i].api_name + "\",this,\"in\")'>一键转入</button><button onclick='yijianinout(\"" + platform[i].api_name + "\",this,\"out\")'>一键转出</button></div></td></tr>";
	}
	$("#limitplace").html(str);
	$("#limitlist").html(arr);
}
            ...
            if (retData != null) {
                for (var i = 0; i < retData.length; i++) {
                    if (retData[i].type == "zfb") {
                       str += '<div class="col col-name tdName">' + retData[i].payname + '</div><div class="col col-data" ><label class="pay-label"><span class="blink">' + $("#userNameController").attr("minmoney") + '-' + $("#userNameController").attr("maxmoney") + '</span><input type="radio" name="payType" value="' + retData[i].payname + '"><img width="266" height="70" src="/Areas/Wap6/Content/images/main/zfbxx.png"></label></div>';
                    }
                }
                $("#skAccount").html(str);
                $("#skAccount input[type=radio]").first().prop("checked", true);
            ...
            if (retData != null) {
                for (var i = 0; i < retData.length; i++) {
                    if (retData[i].type == "qqpay") {
                        str += '<div class="col col-name tdName">' + retData[i].payname + '</div><div class="col col-data" ><label class="pay-label"><span class="blink">' + $("#userNameController").attr("minmoney") + '-' + $("#userNameController").attr("maxmoney") + '</span><input type="radio" name="payType" value="' + retData[i].payname + '"><img width="266" height="70" src="/Areas/Wap6/Content/images/main/qqxx.png"></label></div>';
                    }
                }
                $("#skAccount").html(str);
                $("#skAccount input[type=radio]").first().prop("checked", true);
            ...
            if (retData != null) {
                for (var i = 0; i < retData.length; i++) {
                    if (retData[i].type == "ysf") {
                        str += '<div class="col col-name tdName">' + retData[i].payname + '</div><div class="col col-data" ><label class="pay-label"><span class="blink">' + $("#userNameController").attr("minmoney") + '-' + $("#userNameController").attr("maxmoney") + '</span><input type="radio" name="payType" value="' + retData[i].payname + '"><img width="266" height="70" src="/Areas/Wap6/Content/images/main/ysfxx.png"></label></div>';
                    }
...
function changebank(obj) {
    $("#bankName").html($(obj).val());
    $("#AccountHolders").html($(obj).attr("bankinfo")); 
    $("#BankId").html($(obj).attr("bankId")); 
}
...
function notify(msg) {
    ...
    $("#modalWaring").show();
    $("#modalWaring").addClass("in");
    $("#modalWarings").addClass("in");
    $("#modalWarings").show();
}
//关闭提示框
function sureWaring() {
    $("#messageWaring").html("");
    $("#modalWaring").hide();
    $("#modalWaring").removeClass("in");
    $("#modalWarings").removeClass("in");
    $("#modalWarings").hide();
}
//获取对应的電子列表
...
//推荐游戏显示
function tuijianGame(retDate) {
    var str = "";
    for (var i = 0; i < retDate.length; i++) {
        str += '<li class="ng-scope swiper-slide" onclick="return landing(this);" platformCode="' + retDate[i].api_Name + '" gameType = "' + retDate[i].gameType + '" gameId="' + retDate[i].gameId + '" gameName="' + retDate[i].gameName + '"><div class="pic"><img src= "/img/imgPC/' + retDate[i].api_Name.toLowerCase() + "/" + retDate[i].onlyImg + '.png"></div><div class="text ng-binding">' + retDate[i].chineseName + '</div></li>';
    }

    $("#TJgame-list").html(str);

    var TJgameSwiper = new Swiper('.tj-swiper', {
        slidesPerView:3,
...
//热门游戏显示
function getHotGame(retDate) {
    $("#hotgame-list").html("");
    var str = "";
    for (var i = 0; i < retDate.length; i++) {
        str += '<li class="ng-scope" onclick="return landing(this);" platformCode="' + retDate[i].api_Name + '" gameType = "' + retDate[i].gameType + '" gameId="' + retDate[i].gameId + '" gameName="' + retDate[i].gameName + '"><div class="pic"><img src= "/img/imgPC/' + retDate[i].api_Name.toLowerCase() + "/" + retDate[i].onlyImg + '.png"></div><div class="text ng-binding">' + retDate[i].chineseName + '</div></li>';
    }
    $("#hotgame-list").html(str);
}
//根据ID获取对应的游戏列表
function getFishGame(retDate, type) {
    ...
    for (var i = 0; i < retDate.length; i++) {
        ...
        if (type == "fish") str += '<li><a href="javascript:;" onclick="return landing(this);" platformCode="' + retDate[i].api_Name + '" gameType="' + retDate[i].gameType + '" gameId="' + retDate[i].gameId + '" gameName="' + retDate[i].gameName + '"> <img src="/img/fish/' + retDate[i].api_Name.toLowerCase() + '_' + retDate[i].onlyImg + '.png" style="width:60%"/><span class="ng-binding">' + retDate[i].api_Name + retDate[i].chineseName + '</span> </a></li>';
        else str += '<li class="ng-scope" onclick="return landing(this);" platformCode="' + retDate[i].api_Name + '" gameType = "' + retDate[i].gameType + '" gameId="' + retDate[i].gameId + '" gameName="' + retDate[i].gameName + '"><div class="pic"><img src= "/img/imgPC/' + retDate[i].api_Name.toLowerCase() + "/" + retDate[i].onlyImg + '.png"></div><div class="text ng-binding">' + retDate[i].chineseName + '</div></li>';
...
function addHb() {
    ...
    num++;
    $(".couten").append("<li class='li" + num + "' ><a href='javascript:;'><img src='/Areas/Wap6/Content/images/main/hb_" + hb + ".png'></a></li>");
    $(".li" + num).css({
        "left": Left,
    });
    $(".li" + num + " a img").css({
...
function chooseBlance(obj, money) {
    $(".recommend-section div").removeClass("custom-deposit-bg");
    $(obj).addClass("custom-deposit-bg");
    $(".choosemoney").val(money);
}
function landingslocations(){//页面跳转
    if ($("#userNameController").val() != "") {
		return true;
	} else {
		notify("请先登录！");
