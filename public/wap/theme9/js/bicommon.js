var biCommon = {
    ...
    addCards: function (cardName, cardNum, cardAddress) {//绑定银行卡
        ...
        biCommon.ajax('post', data, "/api/MemberApi?ApiInterface=bindbankadd", function (res) {
            if (res.retCode == 0) {
                notify("绑定银行卡成功");
                setTimeout(function () { window.location.href = "/Index/info"; }, 1000);
            } else {
                notify(res.retMsg);
            }
        })
        ...
        biCommon.ajax('post', data, "/api/RecordingApi?ApiInterface=payinfolist", function (res) {
            if (res.retCode == 0) {
                if (biCommon.isEmptyObject(res.retData) == false) {
                    ...
                    //$("#mgmChongZhi li").eq(0).css({ "display": "block" });
                }
            } else {
                notify(res.retMsg);
            }
    ...
    getGameSlotCount: function (game, gametype, pageIndex, gameName) {//根据平台名称获取对应的電子子游戏
        ...
        var gamePageSizes = gametype == "-3" ? 100 : gamePageSize;
        var data = { "game": game, "gametype": gametype, "pageSize": gamePageSizes, "pageIndex": pageIndex, "devices": "HTML5", "gameName": gameName }
        biCommon.ajax('post', data, "/api/NotLoggedInApi?ApiInterface=getapigame", function (res) {
            if (res.retCode == 0) {
                if (gametype == "-3") {//推荐
                    tuijianGame(res.retData);
                }
                else if (gametype == "-1") {//热门
                    $("#gamePlate").val(game);
                    getHotGame(res.retData);
                }
                else {//全部
                    $("#gamePlate").val(game);
