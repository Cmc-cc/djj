...
@section('content')
	...
	.userInfo dl>dd{background:#fff !important}
	.gm_main .userInfo dt{background:#fff !important}
	</style>
    <div class="m_container">
        <div class="m_body">
            <div class="container-fluid gm_main">
                ...
                {{--</ul>--}}
                {{--</div>--}}
                <div class="userInfo setCard" style="color:#000;">
                    <dl>
                        <dt>账户信息</dt>
                        <dd>
                            <div class="pull-left">
				...
				?>	
                    <form id="form1" name="form1" class="form-horizontal" action="{{ route('wap.post_transfer') }}"
                        ...
                        <select name="out_account" id="out_account" class="form-control"  onChange="changePay1(this.options[this.selectedIndex].value)">
                            <option value="">--请选择--</option>
                            <option value="1">中心账户</option>
                            <option value="2">返水账户</option>
                            @foreach($_api_list as $k => $v)
                                <option value="{{ $k }}">{{ $_api_list_1[$k] }}</option>
                            @endforeach
                        </select>
                    </dd>
                    <dd>
                        ...
                        <select name="in_account" id="in_account" class="form-control"  onChange="changePay2(this.options[this.selectedIndex].value)">
                            <option value="">--请选择--</option>
                            <option value="1">中心账户</option>
                ...
                {{--</script>--}}
				<script>
				  function changePay1(val){
					  console.log(val);
					  if(val=='-1' || val=='-2' ){
						  var options = $('#in_account').find("option");
                          options.first().attr("selected", true);
					  }
				  }
				  function changePay2(val){
					  console.log(val);
					 if(val=='-1' || val=='-2' ){
						  var options = $('#out_account').find("option");
                          options.first().attr("selected", true);
					  } 
				  }
				</script>
