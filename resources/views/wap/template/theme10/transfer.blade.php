	...
	<div class="moneyBag">
			...
			<ul class="moneyInfor">
				<li id="center_money" data-uri="https://www.cash669.net/api/member_balance/4751">
					...
					<p id="gamemoneyy">{{$member->money}}</p>
		...
		<!--<div class="needHelp">如需幫助，請<a class="kefu_box">{{trans("lang.zxkf")}}</a></div>-->
		<div id="screen"></div>
		<div class="choseList" id="chonseOne">
			...
			<ul>
				...
				@foreach($api_list as $item)
					<li data-from="{{ $item->id }}">{{ $item->api_title }}</li>
				@endforeach
			</ul>
		</div>
		<div class="choseList" id="chonseTwo">
			...
			<ul>
				...
				@foreach($api_list as $item)
					<li data-to="{{ $item->id }}">{{ $item->api_title }}</li>
				@endforeach					
			</ul>
		</div>
